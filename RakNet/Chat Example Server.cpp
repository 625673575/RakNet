...
//	server->AttachPlugin(&packetLogger);
	...
	if (kbhit())
	{
		...
		if(strcmp(message,"package")==0)
		{
			/*time_t now_time;
			now_time=time(&now_time);
			sprintf(message, "%s", asctime(gmtime(&now_time)));*/
			SYSTEMTIME sys;
			GetLocalTime(&sys);
			sprintf(message, "%02d %02d %02d %02d",sys.wHour, sys.wMinute,sys.wSecond,sys.wMilliseconds);
		...
		for (p=server->Receive(); p; server->DeallocatePacket(p), p=server->Receive())
		{
			...
			default:
				...
				if(strcmp(message, "fuck")==0)
				{
					whatthefuckCount++;
					printf("Get unreliable Message %d:%s \n",whatthefuckCount,message);
					continue;
				}
				else{
					...
					//if(strcmp(message,"package")==0)
					{
						SYSTEMTIME sys;
						GetLocalTime(&sys);
						sprintf(message, "%s %02d %02d %02d %02d",p->data,sys.wHour, sys.wMinute,sys.wSecond,sys.wMilliseconds);
						server->Send(message, (const int) strlen(message)+1, HIGH_PRIORITY, RELIABLE_ORDERED, 0, RakNet::UNASSIGNED_SYSTEM_ADDRESS, true);
					}
